...
class IpListModel(BaseModel):
    ...
    def validate_purpose(cls, v):
        if v is not None:
            v_trimmed = v.strip()
            if v_trimmed and not re.match(r"^[a-zA-Z0-9\s,.:\/()&<>_-]+$", v_trimmed):
                raise ValueError("Purpose must contain alphanumeric characters, spaces, commas, periods, colons, slashes, parentheses, ampersands, angle brackets, underscores, or dashes only")
