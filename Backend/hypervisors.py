...
@router.get("/", response_description="List all hypervisors", response_model=PaginatedHypervisorsModel, response_model_by_alias=False, dependencies=[Depends(require_privilege("View Configurations"))])
async def list_items(
    ...
    if search:
        query = {
            "hypervisor": {"$regex": search, "$options": "i"}
        }

    actual_sort_by = sortBy or sort_by or "hypervisor"
...
@router.post("/", response_description="Create a hypervisor", response_model=HypervisorModel, status_code=status.HTTP_201_CREATED, response_model_by_alias=False, dependencies=[Depends(require_any_privilege(["Create Configuration", "Create Server Details"]))])
async def create_item(
    payload: CreateHypervisorModel = Body(...),
    current_user: dict = Depends(get_current_user)
):
    existing = await collection.find_one({ "hypervisor": {"$regex": f"^{getattr(payload, 'hypervisor')}$", "$options": "i"} })
    if existing:
        raise HTTPException(status_code=400, detail="Hypervisor already exists")
...
@router.put("/{id}", response_description="Update a hypervisor", response_model=HypervisorModel, response_model_by_alias=False, dependencies=[Depends(require_any_privilege(["Update Configurations", "Update Server Details"]))])
async def update_item(id: str, payload: UpdateHypervisorModel = Body(...)):
    ...
    if len(item_dict) >= 1:
        if "hypervisor" in item_dict:
            existing = await collection.find_one({
                "hypervisor": {"$regex": f"^{item_dict['hypervisor']}$", "$options": "i"},
                "_id": {"$ne": ObjectId(id)}
            })
            if existing:
                raise HTTPException(status_code=400, detail="Hypervisor already exists")
...
@router.delete("/{id}", response_description="Delete a hypervisor", dependencies=[Depends(require_privilege("Delete Configurations"))])
...
@router.post("/bulk", response_description="Bulk create hypervisors", dependencies=[Depends(require_any_privilege(["Create Configuration", "Create Server Details"]))])
