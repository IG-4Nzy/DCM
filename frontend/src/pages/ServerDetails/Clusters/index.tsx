...
const Clusters = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        e?.response?.data?.detail || "Failed to load clusters",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    ...
    if (isConfirmed) {
      try {
        ...
        showToast(
          e?.response?.data?.detail || "Failed to delete Cluster",
          "error",
        );
      }
    }
  ...
  const columns: Column<ClusterData>[] = [
    { id: "slNumber", label: "SL No", sortable: true },
    {
      ...
      render: (row) => {
        const cTypeStr =
          `${row.clusterType || ""} ${row.clusterName || ""}`.toLowerCase();
        let icon = null;
        if (
  ...
  const activeFilterCount =
    (clusterTypeFilter ? 1 : 0) + (networkTypeFilter ? 1 : 0);
  ...
  const handleCsvDownload = () => {
    generateAndDownloadCSV({ data, tab: TAB_CONSTANTS.CLUSTERS });
  };
        ...
        <Table
          ...
          onRowClick={(row) => {
            ...
            window.dispatchEvent(
              new CustomEvent("changeServerDetailsTab", { detail: "nodes" }),
            );
          }}
        />
      </Box>
