...
import {
  ...
  MdWifi as NetworkOnIcon,
  MdWifiOff as NetworkOffIcon,
...
const VMDetails = ({
  ...
  const [selectedHistoryVm, setSelectedHistoryVm] =
    useState<VMDetailsData | null>(null);
  ...
  const hasUpdate =
    ...
    hasPrivilege(PRIVILEGES.UPDATE_VMS_RESTRICTED);
  const hasDelete =
    isSuperuser || hasPrivilege(PRIVILEGES.SERVER_DETAILS_CREATE);
  ...
  const [powerStatusFilter, setPowerStatusFilter] = useTableState(
  ...
  const [clusterTypeFilter, setClusterTypeFilter] = useTableState(
  ...
  useEffect(() => {
    request
      .get("/api/vcenter-details/?pagination=false")
    ...
    if (isConfirmed) {
      ...
      try {
        const res = await request.post(
          "/api/vm-details/import-vcenter",
          {},
          { timeout: 60000 },
        );
        showToast(
          res.data?.message || "Successfully imported VMs from vCenter",
          "success",
        );
        loadData();
      } catch (err: any) {
        const detail =
          ...
          (err?.code === "ECONNABORTED"
            ? "Request timed out. The vCenter import is taking longer than expected."
            : "Failed to import VMs from vCenter");
  ...
  useEffect(() => {
    ...
    request
      .get("/api/cluster-types/", { params: { pagination: false } })
      .then((res) => {
        const types = (res.data?.data || [])
          .map((t: any) => t.clusterType)
          .filter(Boolean)
          .sort();
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      showToast(
        e?.response?.data?.detail || "Failed to load VM Details",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleAddToMonitoring = async (row: VMDetailsData) => {
    ...
    if (!row.ipAddress) {
      showToast(
        "This VM does not have an IP address configured. Edit the VM to set an IP first.",
        "warning",
      );
      return;
    }
  ...
  const handleDelete = async (id: string, ipAddress: string) => {
    const isConfirmed = await confirm(
      `Are you sure you want to delete VM Details for ${ipAddress}? This action cannot be undone.`,
      "Delete VM Details",
    );
    if (isConfirmed) {
      try {
        ...
        showToast(
          e?.response?.data?.detail || "Failed to delete VM Details",
          "error",
        );
      }
    }
  ...
  const handleCsvDownload = () => {
    const csvData = data.map((row: VMDetailsData) => {
      ...
      const foundCluster = clusters.find(
        (c: any) =>
          c.id === clusterId ||
          c._id === clusterId ||
          c.vcenterClusterId === clusterId ||
          (c.clusterName &&
            c.clusterName.toLowerCase() === clusterId.toLowerCase()),
      );
      ...
      const nodeIp =
        foundNode?.ip || foundNode?.ipAddress || foundNode?.managementIp || "";

      const adminName = row.adminName || usersMap[row.adminName] || "--";

      return {
        vmName: row.vmName || "--",
        clusterId: foundCluster?.clusterName || "--",
        ipAddress: row.ipAddress || "--",
        node: nodeIp ? `${nodeName} - ${nodeIp}` : nodeName,
        admin: adminName,
      };
    });
  ...
  return (
    <Box className={styles.container}>
      <Box className={styles.container__header}>
        <Typography variant="h6" className={styles.container__header__label}>
          VM Details
        </Typography>
        <Box
          className={styles.container__header__search}
        ...
        <FilterGroup title="Cluster & Host Node">
          ...
          <Dropdown
            ...
            options={[
              ...
              ...nodesList.map((n: any) => {
                const nodeName =
                  n.node || n.hostName || n.nodeId || n.name || "";
      ...
      <Paper className={styles.tableWrapper}>
        <TableContainer
          sx={{ flexGrow: 1, flexShrink: 1, minHeight: 0, overflow: "auto" }}
        >
          <Table size="medium" stickyHeader>
            <TableHead>
              <TableRow className={styles.tableWrapper__headerRow}>
                <TableCell className={styles.tableWrapper__headerCell}>
                  VM ID
                </TableCell>
                <TableCell className={styles.tableWrapper__headerCell}>
                  VM Name
                ...
                {(hasUpdate || hasDelete) && (
                  <TableCell
                    align="right"
                    className={styles.tableWrapper__headerCellLast}
                  >
                    Actions
                  </TableCell>
                )}
            ...
            <TableBody>
              {loading && data.length === 0 ? (
                ...
                data.map((row) => (
                  ...
                  >
                    <TableCell
                      className={styles.tableWrapper__cell}
                      ...
                      >
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            return (
                              <Tooltip
                                title={`Power Status: ${isPowerOn ? "ON" : "OFF"}`}
                                arrow
                                placement="top"
                              >
                                <Box
                          ...
                          {(() => {
                            const isNetworkConnected =
                              row.isNetworkConnected !== false;
                            return (
                              <Tooltip
                                title={`Network: ${isNetworkConnected ? "Connected" : "Disconnected"}`}
                                arrow
                                placement="top"
                              >
                                <Box
                          ...
                          {(() => {
                            ...
                            const snapNames = snaps
                              .map(
                                (s: any) =>
                                  s.name || s.snapshotName || "Snapshot",
                              )
                              .join(", ");
                    ...
                    <TableCell className={styles.tableWrapper__cell}>
                      ...
                      >
                        ...
                        {(() => {
                          const isInternet = row.networkType
                            ? row.networkType.toLowerCase() === "internet"
                            : row.ipAddress?.startsWith("192.168");
                          const isIntranet = row.networkType
                            ? row.networkType.toLowerCase() === "intranet"
                            : row.ipAddress?.startsWith("10.");
                    ...
                    {(hasUpdate || hasDelete) && (
                      ...
                      >
                        <Box className={styles.tableWrapper__actions}>
                          {!isRestrictedAdmin &&
                            (row.ipAddress &&
                            monitoredIps.has(row.ipAddress) ? (
                              <Tooltip title="Already Monitored">
                                <span>
                                  <IconButton
                                    ...
                                    disabled
                                    className={
                                      styles.tableWrapper__actions__editBtn
                                    }
                                    sx={{
                                      mr: 0.5,
                                      color: "#2e7d32",
                              ...
                              <Tooltip title="Add to Monitoring">
                                <IconButton
                                  ...
                                  onClick={() => handleAddToMonitoring(row)}
                                  sx={{ mr: 0.5 }}
                                >
                                  <MonitorIcon fontSize="small" />
                                </IconButton>
                          ...
                          <Tooltip title="Update History">
                            <IconButton
                              size="small"
                              color="secondary"
                              className={styles.tableWrapper__actions__editBtn}
                              onClick={() => handleOpenUpdateHistory(row)}
                            >
                          ...
                          {hasDelete && (
                            <Tooltip title="Delete">
                              <IconButton
                                ...
                                color="error"
                                className={
                                  styles.tableWrapper__actions__deleteBtn
                                }
                                onClick={() =>
                                  handleDelete(
                                    row.id || "",
