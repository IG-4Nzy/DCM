...
const Inventory: React.FC = () => {
  ...
  const { inventory, loading, totalCount } = useSelector(
    (state: RootState) => state.inventory,
  );
  const { isSuperuser, privileges } = useSelector(
  ...
  const [activeTab, setActiveTab] = useTableState<
    "general" | "returnable" | "assets"
  >("inventory_activeTab", "general");
  ...
  const canClickRow =
    hasUpdate ||
    hasPrivilege(PRIVILEGES.INVENTORY_VIEW_ALL) ||
    hasPrivilege(PRIVILEGES.INVENTORY_VIEW_DEPT);
  ...
  useEffect(() => {
    if (
      ...
      const hasReturnable = inventory.some(
        (item) => item.isReturnable === true,
      );
      if (hasReturnable) {
  ...
  const handleDelete = async (id: string) => {
    if (await confirm("Are you sure you want to delete this item?")) {
      ...
      if (deleteInventory.fulfilled.match(result)) {
        ...
        showToast(
          (result.payload as string) || "Failed to delete item",
          "error",
        );
      }
      dispatch(fetchInventory(getFetchParams()));
  ...
  const handleFormSubmit = async (data: any) => {
    if (selectedItem) {
      ...
      if (editInventoryItem.fulfilled.match(result)) {
        ...
        showToast(
          (result.payload as string) || "Failed to update item",
          "error",
        );
      }
    } else {
      ...
      if (createInventory.fulfilled.match(result)) {
        ...
        showToast(
          (result.payload as string) || "Failed to create item",
          "error",
        );
      }
    }
  ...
  const handleBulkUpload = async (
    ...
    if (bulkCreateInventory.fulfilled.match(resultAction)) {
      ...
      showToast(
        (resultAction.payload as string) || "Bulk upload failed",
        "error",
      );
    }
    // reset input
  ...
  useEffect(() => {
    if (selectedItem) {
      const updatedItem = inventory.find(
        (i) => (i.id || i._id) === (selectedItem.id || selectedItem._id),
      );
      if (updatedItem) {
  ...
  columns.push(
    { id: "description", label: "Description", sortable: false },
    {
      ...
      render: (row: any) => {
        ...
        const cleaned = row.lastUpdatedDate
          .replace(/\+00:00Z$/, "Z")
          .replace(/\+00:00$/, "Z");
      ...
      >
        <Table
          ...
          onRowClick={
            canClickRow
              ? (row) => {
                  setSelectedItem(row as InventoryData);
