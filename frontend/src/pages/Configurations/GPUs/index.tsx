...
import { fetchGPUs, createGPU, updateGPU, deleteGPU } from './action';
import { type GPUData } from './model';
...
const GPUs = () => {
    ...
    const [editingItem, setEditingItem] = useState<GPUData | null>(null);
    ...
    const [searchQuery, setSearchQuery] = useTableState('GPUs_search', '');
    const [page, setPage] = useTableState('GPUs_page', 0);
    const [rowsPerPage, setRowsPerPage] = useTableState('GPUs_rowsPerPage', 5);
    const [order, setOrder] = useTableState<Order>('GPUs_order', 'asc');
    const [orderBy, setOrderBy] = useTableState<string>('GPUs_orderBy', 'gpuName');
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            showToast(e?.response?.data?.detail || 'Failed to load GPUs', 'error');
        } finally {
            setLoading(false);
        }
    ...
    const handleOpenModal = (item?: GPUData) => {
    ...
    const handleSubmit = async (payload: any) => {
        try {
            ...
            showToast(e?.response?.data?.detail || 'Failed to save GPU', 'error');
        }
    };

    const handleDelete = async (item: GPUData) => {
        const isConfirmed = await confirm(`Are you sure you want to delete ${item.gpuName}?`, 'Delete GPU');
        if (isConfirmed) {
            try {
                await deleteGPU(item.id);
                ...
                showToast(e?.response?.data?.detail || 'Failed to delete GPU', 'error');
            }
        }
    };
    ...
    const columns: Column<GPUData>[] = [
        { id: 'gpuName', label: 'GPU Name', sortable: true },
