...
const { MenuIcon, LogoutIcon, ChevronLeftIcon, ChevronRightIcon } = Icons;
...
import { fetchDashboardSummary } from "../pages/Dashboard/action";
...
const Layout: React.FC = () => {
  ...
  useEffect(() => {
    if (username) {
      const notifyVisit = async () => {
        const currentOption = SIDEBAR_OPTIONS.find((opt) =>
          location.pathname.startsWith(opt.route),
        );
        if (currentOption) {
          try {
            await request.post("/api/notifications/visit", {
              route: currentOption.route,
            });
            setUnreadRoutes((prev) => ({
              ...prev,
              [currentOption.route]: false,
            }));
          } catch (err) {
  ...
  useEffect(() => {
    const fetchInitials = async () => {
      try {
        ...
        setUserInitials(
          l ? `${f}${l}` : f || (username || "?").charAt(0).toUpperCase(),
        );
        setUserFullName(
  ...
  useEffect(() => {
    ...
    if (currentOption) {
      if (
        !isSuperuser &&
        currentOption.privileges &&
        !hasAnyPrivilege(currentOption.privileges)
      ) {
        const firstAvailable = SIDEBAR_OPTIONS.find(
          (opt) =>
            isSuperuser || !opt.privileges || hasAnyPrivilege(opt.privileges),
        );
        if (firstAvailable) {
        ...
        >
            ...
            <List>
              {SIDEBAR_OPTIONS?.map((option: any, index: number) => {
                ...
                if (
                  !isSuperuser &&
                  option.privileges &&
                  !hasAnyPrivilege(option.privileges)
                ) {
                  return null;
                }
          ...
          <List>
            <ListItem disablePadding sx={{ display: "block" }}>
              <ListItemButton
                onClick={() =>
                  window.dispatchEvent(new Event("openStickyNote"))
                }
                sx={{
                  minHeight: 48,
                  justifyContent: open ? "initial" : "center",
